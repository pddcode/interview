...
> 接收端的缓存传输数据给应用层，但这个过程不一定是即时的，如果发送速度太快，会出现接收端数据overflow，流量控制就是解决这个问题。

> 接收端将自己可以接收的缓冲区大小放入TCP首部的窗口大小字段中。

> 对于发送窗口来说，包含两部分，即已发送还未收到ACK，可以发送但还未发送，图中发送窗口大小是20：
![](https://img-blog.csdn.net/20160906081402924)

### 停止等待协议
##### 确认丢失
当A发送M1消息，B收到后，B向A发送了一个M1确认消息，但却在传输中丢失，而A并不知道，在超时计时过后，A重传M1消息，B再次收到该消息后采取以下措施：
- 丢弃重复的M1消息，不向上层交付
- 向A发送确认消息
...
##### 自动重传请求ARQ协议
每次发完都使用计时器等待，时间超过了就重发，效率很低

##### 连续ARQ协议
发送方维持一个发送窗口，凡是位于发送窗口内的分组可以连续发送出去，而不需要等待对方的确认，接收方一般采用累积确认，对按序到达的最后一个分组发送确认，表明这个分组为止的所有分组都已经正确收到了。
...
### Masonry
1.首先masonry布局的一个简单的例子：view1.left.equalto(view2.left).mas_offset(20)，那么这句代码换一种表示形式就是view1.attribute1 = multiple * view2.attribute2 + constant，那么这就是一个自动布局的约束了。

2.在上面的等式中，view1.attribute1这个在Masonry中是用MASViewAttribute类的对象来表示，这个对象有view和attributelayout属性，分别被view1和attribute1赋值，同理，右边的view2.attribute2也被封装为一个MASViewAttribute对象，然后这一整个等式是一个MASViewConstraint 对象，这个对象有firstAttribute和secondAttribute,layoutRelation，layou
 constant属性，分别被赋值。这样一个MASViewConstraint对象就创建好了。

3.MASConstraintMaker是一个管理类，masviewconstaint对象创建好以后会被加入masconstaintmaker的数组中，最后会去遍历这个数组，把每个masviewconstraint转化为一个NSLayoutConstraint对象，剩下的就交给系统去处理了。

4.MASCompositeConstraint是一个组合约束类，我们设置size这种组合属性的时候会创建这个对象，这个内部也是把一个组合约束拆分成多个单个约束。

### 修改抽屉类型，侧滑的设置里面有滑杆，滑杆滑动不灵，很容易导致返回，一开始直觉觉得是响应区域太小了，把响应区域变大点就可以了，于是就写了一个uislider的子类，重写hittest和pointinside方法，这样做了之后发现还是不管用，然后我就上stack overflow搜了这个问题，
...
由于我这里抽屉是用的第三方库，我想到这里实现抽屉是不是用了UI scrollview，所以就看了一下源码，发现并没有用，然后我就仔细看了一下源码，发现用了pangesture来实现页面的滑动，显然这里盘 gesture是和uislider冲突了，我需要让接触点在uislider上面时，不去响应盘 gesture的事件，于是就去第三方框架里面修改了以下方法：
-(BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch{
    if([touch.view isKindOfClass:[UISlider class]]){
	return NO;
    }

传统慕课录制存在文件数据量大的缺点，要解决慕课文件数据量大的问题，最常见的办法是采用压缩率更高的视频压缩算法，如目前的h264视频编解码算法，一般情况下可以实现1:10的压缩比率，但是大部门的视频压缩算法属于有损压缩算法，也就是说在视频压缩的过程中会丢失大部门视频数据，表现为视频清晰度降低，对于慕课来说会产生很多问题，因此视频压缩率有下限值。
...
### delegate block 的关系，区别，以及notification和线程的问题
...
ssl使用非对称加密方式来传递对称加密所使用的密钥，然后通过对称加密来加密报文。
